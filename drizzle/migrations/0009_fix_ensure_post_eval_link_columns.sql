...
BEGIN
  ...
  SELECT a.branch_id INTO v_branch FROM public.appointments a WHERE a.id = v_survey.booking_id;
  ...
  ELSIF v_note.post_eval_token IS NULL THEN
    UPDATE public.pre_service_counseling_notes n
    SET post_eval_token = gen_random_uuid()
    WHERE n.id = v_note.id
    RETURNING * INTO v_note;
  END IF;

  v_counselor := COALESCE(v_note.assigned_counselor_id, v_note.updated_by);
  ...
  SELECT
    ...
    b.name_th,
    b.name_en,
    COALESCE(p.display_name, sp.name_th, sp.name_en),
  ...
  LEFT JOIN public.profiles p ON p.id = v_counselor
  LEFT JOIN public.staff_profiles sp ON sp.user_id = v_counselor;
END;
$function$;
