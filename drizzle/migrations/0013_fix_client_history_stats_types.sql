...
BEGIN
  ...
  SELECT COUNT(*)::bigint,
         COALESCE(SUM(full_rows.v), 0)::bigint,
         COUNT(*) FILTER (WHERE full_rows.v > 1)::bigint,
         COUNT(*) FILTER (WHERE full_rows.counseled = 1 AND full_rows.has_eval = 0)::bigint,
         COUNT(*) FILTER (WHERE full_rows.has_eval = 1)::bigint,
         COUNT(*) FILTER (WHERE full_rows.counseled = 0 AND full_rows.reviewed = 1)::bigint,
         COUNT(*) FILTER (WHERE full_rows.counseled = 0 AND full_rows.reviewed = 0)::bigint
  FROM full_rows;
END;
$$;