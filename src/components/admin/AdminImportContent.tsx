...
import {
  Upload, FileSpreadsheet, Play, Eye, Loader2, CheckCircle2, AlertTriangle, Download, Database, FileSearch, History
} from "lucide-react";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { Label } from "@/components/ui/label";
import { supabase } from "@/integrations/supabase/client";
import { toast } from "sonner";
import ImportHistoryTab from "./import/ImportHistoryTab";
...
export default function AdminImportContent() {
  ...
  const runImport = async (dryRun: boolean) => {
    if (!selectedFile) { toast.error(language === "th" ? "กรุณาเลือกไฟล์ CSV" : "Please select a CSV file"); return; }
    setLoading(true); setMode(dryRun ? "dry_run" : "import"); setResult(null); setDbVerified(null);
    ...
    try {
      ...
      const response = await supabase.functions.invoke("import-existing-selftest-csv", { body: formData });
      ...
      if (!dryRun && data.result) {
        ...
        toast.success(language === "th" ? `นำเข้าสำเร็จ — สถิติอัปเดตแล้ว` : `Import complete — stats updated`);
        window.dispatchEvent(new CustomEvent("selftest-import-complete"));
      } else if (dryRun) {
        toast.info(language === "th" ? "ตรวจสอบเสร็จ (ยังไม่บันทึก)" : "Dry run complete (no data saved)");
      }
    } catch (err: any) {
      console.error("Import error:", err);
      toast.error(err.message || "Import failed");
    } finally {
      setLoading(false); setMode("idle");
    }
  };
  ...
  const downloadErrorReport = () => {
    ...
    a.href = url; a.download = `import-errors-${new Date().toISOString().slice(0, 10)}.csv`; a.click();
        ...
        <div className="space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex items-center gap-3">
                <Label className="text-sm font-medium shrink-0">{language === "th" ? "สาขา:" : "Branch:"}</Label>
              ...
              {selectedFile && (
                <div className="flex gap-3">
                  <Button variant="outline" onClick={() => runImport(true)} disabled={loading}>
                    {loading && mode === "dry_run" ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <Eye className="h-4 w-4 mr-2" />}
                    {language === "th" ? "ตรวจสอบ (Dry Run)" : "Dry Run"}
                  </Button>
                  <Button onClick={() => runImport(false)} disabled={loading}>
                    {loading && mode === "import" ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <Play className="h-4 w-4 mr-2" />}
                    {language === "th" ? "นำเข้าข้อมูล" : "Import"}
                  </Button>
                </div>
              )}
          ...
          {result && (
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  {isDryRun ? <Eye className="h-5 w-5 text-blue-500" /> : <CheckCircle2 className="h-5 w-5 text-green-500" />}
                  {isDryRun
              ...
              <CardContent className="space-y-4">
                <div className="flex items-center gap-2 p-3 rounded-lg bg-muted/50 border border-border/50">
                  ...
                  <span className="text-sm font-medium">{language === "th" ? "รูปแบบ CSV:" : "CSV type:"}</span>
                  <Badge variant="secondary" className="text-xs">{csvTypeLabel(result.csvType)}</Badge>
                </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                  ...
                  <div className="p-3 rounded-lg bg-green-500/10 text-center">
                    ...
                    <div className="text-xs text-muted-foreground">{isDryRun ? (language === "th" ? "จะเพิ่มใหม่" : "Will Insert") : (language === "th" ? "เพิ่มใหม่" : "Inserted")}</div>
                  </div>
                  <div className="p-3 rounded-lg bg-blue-500/10 text-center">
                    <div className="text-2xl font-bold text-blue-600">{result.updated}</div>
                    <div className="text-xs text-muted-foreground">{isDryRun ? (language === "th" ? "จะอัปเดต" : "Will Update") : (language === "th" ? "อัปเดต" : "Updated")}</div>
                  </div>
                  ...
                  <div className="p-3 rounded-lg bg-red-500/10 text-center">
                    ...
                    <div className="text-xs text-muted-foreground">{language === "th" ? "ข้อผิดพลาด" : "Errors"}</div>
                  </div>
                </div>
                ...
                {!isDryRun && dbVerified !== null && (
                  <div className="flex items-center gap-2 p-3 rounded-lg bg-green-500/10 border border-green-500/20">
                    ...
                    <span className="text-sm font-medium text-green-700">DB verified: {dbVerified} records confirmed</span>
                  </div>
                )}
                  ...
                  <div className="space-y-2">
                    ...
                    <ScrollArea className="h-48 rounded border p-2">
                      ...
                      {result.errors.length > 50 && (
                        <p className="text-xs text-muted-foreground mt-2">... and {result.errors.length - 50} more</p>
                      )}
                    </ScrollArea>
                  </div>
                )}
