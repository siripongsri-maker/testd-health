-- Fix appointment log type mismatch and guest checkout status handling
...
BEGIN
  ...
  VALUES (p_appointment_id, 'self_checkin', auth.uid(), 'Client self check-in');
END;
$function$;
...
BEGIN
  ...
  VALUES (
    ...
    'guest_self_checkin',
    NULL,
    'Guest self check-in (' || upper(trim(p_referral_code)) || ')'
  );
...
BEGIN
  ...
  IF v_appointment.status NOT IN ('arrived', 'waiting', 'in_progress', 'completed') THEN
  ...
  VALUES (
    ...
    'guest_self_checkout',
    NULL,
    'Guest self check-out (' || upper(trim(p_referral_code)) || ')' || CASE WHEN p_rating IS NOT NULL THEN ' (rating: ' || p_rating || '/5)' ELSE '' END
  );
