...
-- Daily booking cap (e.g. dry-run launches)
CREATE TABLE IF NOT EXISTS public.booking_daily_caps (
  ...
  UNIQUE (branch_id, cap_date)
);

ALTER TABLE public.booking_daily_caps ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Anyone can read daily caps" ON public.booking_daily_caps
  FOR SELECT TO anon, authenticated USING (true);

CREATE POLICY "Admins can insert daily caps" ON public.booking_daily_caps
  FOR INSERT TO authenticated WITH CHECK (has_role(auth.uid(), 'admin'::app_role));

CREATE POLICY "Admins can update daily caps" ON public.booking_daily_caps
  FOR UPDATE TO authenticated USING (has_role(auth.uid(), 'admin'::app_role));

CREATE POLICY "Admins can delete daily caps" ON public.booking_daily_caps
  FOR DELETE TO authenticated USING (has_role(auth.uid(), 'admin'::app_role));

CREATE TRIGGER update_booking_daily_caps_updated_at
  BEFORE UPDATE ON public.booking_daily_caps
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

-- Update get_available_slots_dbg to enforce daily cap.
-- When total booked appointments for the branch/date >= max_bookings,
-- return a single closed-day row with closure title "เต็มแล้ว".
...
BEGIN
  -- Daily cap gate (dry-run / soft-launch limited capacity)
...
-- Update check_slot_available to also enforce daily cap
CREATE OR REPLACE FUNCTION public.check_slot_available(p_branch_id uuid, p_date date, p_time time without time zone)
 RETURNS boolean
 LANGUAGE plpgsql
...
BEGIN
  -- Daily cap gate
  SELECT max_bookings INTO v_daily_cap
  FROM public.booking_daily_caps
  WHERE branch_id = p_branch_id AND cap_date = p_date
  LIMIT 1;

  IF v_daily_cap IS NOT NULL THEN
    SELECT COUNT(*)::INTEGER INTO v_day_total
    FROM public.appointments a
    WHERE a.branch_id = p_branch_id AND a.appointment_date = p_date
      AND a.status NOT IN ('cancelled', 'no_show');
    IF v_day_total >= v_daily_cap THEN
      RETURN FALSE;
    END IF;
  END IF;
  ...
  ELSE
    ...
    FROM public.booking_branches bb WHERE bb.id = p_branch_id AND bb.is_active = TRUE;
  ...
  SELECT counselor_count INTO v_capacity FROM public.booking_branches WHERE id = p_branch_id;
  SELECT COUNT(*)::INTEGER INTO v_booked
  FROM public.appointments a
  WHERE a.branch_id = p_branch_id AND a.appointment_date = p_date
    AND a.start_time = p_time AND a.status NOT IN ('cancelled', 'no_show');

  RETURN v_booked < COALESCE(v_capacity, 1);
END;
$function$;
