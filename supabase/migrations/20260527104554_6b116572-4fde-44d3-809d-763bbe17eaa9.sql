...
-- Add uic_display column for admin/internal use (unmasked, alongside uic_code)
ALTER TABLE public.appointment_pre_service_surveys
  ADD COLUMN IF NOT EXISTS uic_display text;

-- Backfill from existing uic_code
UPDATE public.appointment_pre_service_surveys
  SET uic_display = uic_code
  WHERE uic_display IS NULL AND uic_code IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_apsv_uic_display ON public.appointment_pre_service_surveys(uic_display);

-- Update RPC to also persist uic_display
CREATE OR REPLACE FUNCTION public.submit_pre_service_survey(
  p_booking_id uuid, p_uic_code text, p_uic_hash text, p_language text, p_channel text,
  p_knowledge jsonb, p_behavior jsonb, p_confidence integer, p_safety integer,
  p_recommend text, p_mental_health_interest text, p_suggestions text
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  BEGIN
    INSERT INTO public.appointment_pre_service_surveys (
      booking_id, uic_code, uic_display, uic_hash, language, channel,
      knowledge, behavior, confidence, safety, recommend,
      mental_health_interest, suggestions,
      visit_sequence, linked_previous_count
    ) VALUES (
      p_booking_id, p_uic_code, p_uic_code, p_uic_hash, COALESCE(p_language, 'th'), COALESCE(p_channel, 'clinic'),
