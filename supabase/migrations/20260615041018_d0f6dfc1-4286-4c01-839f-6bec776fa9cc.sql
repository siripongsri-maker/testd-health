CREATE OR REPLACE FUNCTION public.submit_guest_selftest_result(p_full_name text, p_phone text, p_line_id text, p_self_result text, p_photo_path text, p_wants_callback boolean)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  END IF;
  -- Photo is optional now. If provided, it must live in the guest/ prefix.
  ...
  INSERT INTO public.hiv_selftest_requests (
    user_id, full_name, phone, line_id,
    self_reported_result, test_result, result_photo_url,
    photo_provided, wants_callback, callback_phone,
    status, submission_path, result_submitted_at
  ) VALUES (
    NULL, v_name, v_phone, nullif(btrim(coalesce(p_line_id, '')), ''),
    v_result, v_result, v_photo,
    (v_photo IS NOT NULL),
    coalesce(p_wants_callback, false),
    CASE WHEN coalesce(p_wants_callback, false) THEN v_phone ELSE NULL END,
    'result_submitted', 'guest_existing_kit', now()
  )
