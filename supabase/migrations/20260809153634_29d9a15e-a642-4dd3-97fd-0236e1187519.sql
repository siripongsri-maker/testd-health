...
BEGIN
  ...
  IF v_nat IS NOT NULL AND v_nat NOT IN ('thai','myanmar','lao','cambodian','vietnamese','other','prefer_not_to_say') THEN
    v_nat := NULL;
  END IF;
  ...
  BEGIN
    INSERT INTO public.appointment_pre_service_surveys (
      ...
      visit_sequence, linked_previous_count, nationality
    ) VALUES (
      ...
      COALESCE(v_seq, 1), COALESCE(v_prev_count, 0), v_nat
    )
