...
-- 1) Harden appointment column protection (staff-only fields)
...
BEGIN
  ...
  NEW.attended_by             := OLD.attended_by;
  NEW.auto_checked_out_at     := OLD.auto_checked_out_at;
  ...
  SELECT jsonb_build_object(
      ...
      FROM (
        SELECT jsonb_build_object(
          ...
          'blocked', EXISTS (
            ...
            WHERE d::date BETWEEN bb.start_date AND COALESCE(bb.end_date, bb.start_date)
          )
        ) AS x
        ...
        LEFT JOIN LATERAL (
          SELECT count(*) AS total,
                 ...
                 (SELECT to_char(a2.start_time, 'HH24:MI') FROM appointments a2
                   WHERE a2.appointment_date = d::date AND a2.status NOT IN ('cancelled')
                   GROUP BY a2.start_time ORDER BY count(*) DESC, a2.start_time LIMIT 1) AS peak_hour
          FROM appointments a
          WHERE a.appointment_date = d::date AND a.status NOT IN ('cancelled')
        ) t ON TRUE
      ) s
    ), '[]'::jsonb),
    ...
    'branch_load', COALESCE((
      ...
      LEFT JOIN LATERAL (
        ...
        FROM appointments a
        WHERE a.branch_id = b.id AND a.status NOT IN ('cancelled')
      ) c ON TRUE
...
REVOKE ALL ON FUNCTION public.get_ops_dashboard() FROM public;
